...
python -m pip install croniter pytest-asyncio httpx websockets
```
...
```bash
ipilot agent -m "Reply with exactly: hello"
```

单轮流式输出：

```bash
ipilot agent -m "Tell me a short story" --stream
```

启动 heartbeat，轮询 `workspace/cron/jobs.json` 中的任务：

```bash
ipilot heartbeat --interval-seconds 30
```
...
```bash
uvicorn ipilot.api.server:app --reload --port 8900
```
